...
import cloud.tamacat.httpd.tomcat.util.ServerUtils;
...
public class TomcatDeployment {
	...
	static final Log LOG = LogFactory.getLog(TomcatDeployment.class);
	...
	public void deploy(ServiceConfig serviceConfig) {
		...
		if (useWarDeploy) {
			deployWarFiles(serviceConfig);
		}

		deployWebapps(serviceConfig);		
	}
	 ...
	 * @param serviceUrl
	 */
	protected void deployWebapps(ServiceConfig serviceConfig) {
		try {	    	
	 ...
	 * @param serviceUrl
	 */
	protected void deployWarFiles(ServiceConfig serviceConfig) {		
		try {
